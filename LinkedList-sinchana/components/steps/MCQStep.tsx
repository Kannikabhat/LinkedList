...
// ✅ Generic interface (decoupled from lessons-data.ts)
export interface GenericMCQOption {
  id: string;
  text: string;
  isCorrect: boolean;
}

export interface GenericMCQ {
  id: string;
  question: string;
  options: GenericMCQOption[];
  explanation: string;
}
...
export default function MCQStep({ step, onAnswer }: MCQStepProps) {
  ...
  if (!step.mcq) {
    return <div>No MCQ data found for this step.</div>;
  }
  ...
  const selectedOptionData = mcq.options.find(
        ...
        <CardContent className="p-8">
          ...
          <div className="space-y-4">
            {mcq.options.map((option) => {
              let buttonClass =
                "w-full p-6 text-left text-lg rounded-lg border-2 transition-all duration-200 ";

...
// Extended interface to support hashing steps with direct slot data
export interface HashingExecutionStep extends GenericExecutionStep {
  slots?: HashSlot[];
}
...
export default function VisualizationStep({ step }: VisualizationStepProps) {
  ...
  // Properly map and normalize slots for hash execution
  const hashExecutionSteps: HashExecutionStep[] = useMemo(
    () =>
      (executionSteps as HashingExecutionStep[])
        .filter(
          (s) =>
            (s.slots && s.slots.length > 0) ||
            (s.visualization?.slots?.length ?? 0) > 0
        )
        .map((s) => ({
          ...(s as HashExecutionStep),
          slots: s.slots ?? s.visualization?.slots ?? [],
        })),
    [executionSteps]
  );
        ...
        <div className="space-y-4">
          ...
          {isHashLesson ? (
            ...
            <LinkedListVisualization
              executionSteps={executionSteps as any[]} // cast to match new prop
              currentStepIndex={currentStepIndex}
            />
          )}
        </div>

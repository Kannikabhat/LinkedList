...
// export interface HashExecutionStep extends HashVisualizationStep {
//   lineIndex: number;
//   key?: string;
...
//   // 🔹 Determine what to render — dynamic steps or static visualization
//   const steps = useMemo(() => {
//     if (executionSteps.length > 0) return executionSteps;
//     if (visualization)
//       return [
//         {
//           ...visualization,
...
//       ];
//     return [];
//   }, [executionSteps, visualization]);

//   const [currentStep, setCurrentStep] = useState<HashExecutionStep>(
//     steps[currentStepIndex] || { slots: [], lineIndex: 0 }
//   );

//   // 🔹 React to step changes dynamically
//   useEffect(() => {
...
//     currentStep.activeSlotIndex ??
//     slots.find((s) => s.isActive)?.index ??
//     undefined;

//   // 🔹 Prepare slots/nodes with active and collision indicators
...
//       ...node,
//       isActive:
//         node.isActive || currentStep.activeNodeKey === node.key || false,
//     })),
//   }));
...
//                   width={slotWidth}
//                   height={slotHeight}
//                   fill={slot.isActive ? "#dbeafe" : "white"}
//                   stroke={slot.isActive ? "#3b82f6" : "#9ca3af"}
//                   strokeWidth={slot.isActive ? 3 : 2}
//                   rx={4}
//                 />
//                 <text
//                   x={slot.x + 15}
...
//                     const nodeX =
//                       slot.x + slotWidth + 20 + nodeIdx * (nodeWidth + 10);
...
export default function HashVisualization({
  ...
  const slots = currentStep.slots || [];
  const message = 'message' in currentStep ? currentStep.message : undefined;
  const hashValue = 'hashValue' in currentStep ? currentStep.hashValue : undefined;
  const code = 'code' in currentStep ? currentStep.code : undefined;
